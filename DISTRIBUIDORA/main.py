...
class Tienda():
    db_name = 'db_coati_care.db'

    def __init__(self, ventana_producto):
        menubar = Menu(ventana_producto)
        ventana_producto.title("COATI CARE SISTEM")
        ventana_producto.geometry("770x710")
        ...
        # Iconos
        self.img_registrar = PhotoImage(
            file="D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/registrar.png")

        self.img_buscar = PhotoImage(
            file="D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/buscar.png")
        self.img_ventas = PhotoImage(
            file="D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/ventas.png")
        self.img_nueva_venta = PhotoImage(
            file="D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/nueva_venta.png")
        self.img_cliente = PhotoImage(
            file="D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/cliente.png")
        self.img_codigo = PhotoImage(
            file="D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/codigo.png")
        self.img_informacion = PhotoImage(
            file="D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/informacion.png")
        # Acciones de menu
        self.boton_registrar = Productos.add_command(
            label="Registrar", command=self.widgets_crud, image=self.img_registrar, compound=LEFT)
        self.boton_buscar = Productos.add_command(
            label="Buscar", command=self.widgets_buscador, image=self.img_buscar, compound=LEFT)
        self.boton_nueva_venta = Ventas.add_command(
            label="Nueva Venta", command=self.widgets_nueva_venta, image=self.img_nueva_venta, compound=LEFT)
        self.boton_ventas = Ventas.add_command(
            label="Ventas", command=self.widgets_ventas, image=self.img_ventas, compound=LEFT)
        self.boton_cliente = Ventas.add_command(
            label="Clientes", command=self.widgets_cliente, image=self.img_cliente, compound=LEFT)
        self.boton_informacion = Informacion.add_command(
            label="Contacto", command=self.widgets_codigos, image=self.img_codigo, compound=LEFT)
        self.boton_informacion = Informacion.add_command(
            label="Informacion del sistema", command=self.widgets_informacion, image=self.img_informacion, compound=LEFT)

        "---------------------Widgets---------------------------"
        # widgets crud
        self.frame_logo_productos = LabelFrame(ventana_producto)
        self.frame_registro = LabelFrame(
            ventana_producto, text="Registrar producto", font=("Arial", 10, "bold"), pady=5)
        ...
        self.frame_buscar_producto = LabelFrame(
            ventana_producto, text="Buscar producto", font=("Arial", 10, "bold"), pady=10)
        ...
        self.frame_nueva_venta = LabelFrame(
            ventana_producto, text="Nueva venta", font=("Arial", 10, "bold"), pady=10)
        self.frame_finalizar_venta = LabelFrame(ventana_producto)
        self.frame_tabla_nueva_venta = LabelFrame(ventana_producto)
        # widgets ventas
        self.frame_buscar_ventas = LabelFrame(
            ventana_producto, text="Buscar venta", font=("Arial", 10, "bold"), pady=10)
        self.frame_boton_buscar_ventas = LabelFrame(ventana_producto)
        self.frame_tabla_ventas = LabelFrame(ventana_producto)
        # widgets cliente
        self.frame_nuevo_cliente = LabelFrame(
            ventana_producto, text="Nuevo Cliente", font=("Arial", 10, "bold"), pady=10)
        self.frame_botones_registro_cliente = LabelFrame(ventana_producto)
        self.frame_buscar_cliente = LabelFrame(
            ventana_producto, text="Buscar Cliente", font=("Arial", 10, "bold"), pady=10)
    ...
    def widgets_crud(self):
        ...
        # Logo
        imagen = Image.open(
            "D:/TSDS/TSDS 2DO AÑO/PRACTICA PROFESIONAL II/LOGIN-USUARIO/DISTRIBUIDORA/img/logo.png")
        ...
        "--------------- Formulario --------------------"
        label_codigo = Label(self.frame_registro, text="Codigo del producto: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=8)
        self.codigo = Entry(self.frame_registro, width=25)
        self.codigo.focus()
        self.codigo.grid(row=0, column=1, padx=5, pady=8)

        label_nombre = Label(self.frame_registro, text="Nombre del producto: ", font=(
            "Arial", 10, "bold")).grid(row=1, column=0, sticky='s', padx=5, pady=8)
        self.nombre = Entry(self.frame_registro, width=25)
        self.nombre.grid(row=1, column=1, padx=5, pady=8)

        label_categoria = Label(self.frame_registro, text="Categoria: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=0, sticky='s', padx=5, pady=9)
        self.combo_categoria = ttk.Combobox(self.frame_registro, values=[
                                            "S/ALCOHOL", "C/ALCOHOL"], width=22, state="readonly")
        self.combo_categoria.current(0)
        self.combo_categoria.grid(row=2, column=1, padx=5, pady=0)

        label_cantidad = Label(self.frame_registro, text="Cantidad: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=2, sticky='s', padx=5, pady=8)
        self.cantidad = Entry(self.frame_registro, width=25)
        self.cantidad.grid(row=0, column=3, padx=5, pady=8)

        label_precio = Label(self.frame_registro, text="Precio ($): ", font=(
            "Arial", 10, "bold")).grid(row=1, column=2, sticky='s', padx=5, pady=8)
        self.precio = Entry(self.frame_registro, width=25)
        self.precio.grid(row=1, column=3, padx=5, pady=8)

        label_descripcion = Label(self.frame_registro, text="Descripcion: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=2, sticky='s', padx=10, pady=8)
        ...
        boton_registrar = Button(self.frame_botones_registro, text="REGISTRAR", command=self.Agregar_producto, height=2,
                                 width=12, bg="green", fg="white", font=("Arial", 10, "bold")).grid(row=0, column=1, padx=10, pady=10)
        boton_editar = Button(self.frame_botones_registro, text="EDITAR", command=self.Editar_producto, height=2,
                              width=12, bg="gray", fg="white", font=("Arial", 10, "bold")).grid(row=0, column=2, padx=10, pady=10)
        boton_eliminar = Button(self.frame_botones_registro, text="ELIMINAR", command=self.Eliminar_producto, height=2,
                                width=12, bg="red", fg="white", font=("Arial", 10, "bold")).grid(row=0, column=3, padx=10, pady=10)

        "--------------- Tabla --------------------"
        self.frame_tabla_crud.config(bd=2)
        self.frame_tabla_crud.grid(row=3, column=0, padx=5, pady=5)

        self.tree = ttk.Treeview(self.frame_tabla_crud, height=11, columns=(
    ...
    def widgets_buscador(self):
        ...
        "--------------- Formulario Buscar--------------------"
        self.label_buscar = Label(self.frame_buscar_producto, text="Buscar Por: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=5)
        self.combo_buscar = ttk.Combobox(self.frame_buscar_producto, values=[
                                         "Codigo", "Nombre"], width=22, state="readonly")
        self.combo_buscar.current(0)
        self.combo_buscar.grid(row=0, column=1, padx=5, pady=5)

        label_codigo_codigo = Label(self.frame_buscar_producto, text="Codigo / Nombre del producto: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=2, sticky='s', padx=5, pady=5)
        ...
        "--------------- Boton --------------------"
        self.boton_buscar = Button(self.frame_boton_buscar, text="BUSCAR", command=self.Buscar_productos,
                                   height=2, width=20, bg="black", fg="white", font=("Arial", 10, "bold"))
        ...
        self.tree_buscar = ttk.Treeview(self.frame_tabla_buscador, height=11, columns=(
            "columna1", "columna2", "columna3", "columna4", "columna5"))
    ...
    def widgets_cliente(self):
        ...
        "--------------- Registrar cliente --------------------"
        self.label_dni = Label(self.frame_nuevo_cliente, text="DNI: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=5)
        self.dni = Entry(self.frame_nuevo_cliente, width=25)
        self.dni.focus()
        self.dni.grid(row=0, column=1, padx=5, pady=5)

        self.label_nombres = Label(self.frame_nuevo_cliente, text="Nombres: ", font=(
            "Arial", 10, "bold")).grid(row=1, column=0, sticky='s', padx=5, pady=5)
        self.nombres = Entry(self.frame_nuevo_cliente, width=25)
        self.nombres.grid(row=1, column=1, padx=10, pady=5)

        self.label_apellidos = Label(self.frame_nuevo_cliente, text="Apellidos: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=0, sticky='s', padx=5, pady=5)
        self.apellidos = Entry(self.frame_nuevo_cliente, width=25)
        self.apellidos.grid(row=2, column=1, padx=10, pady=5)

        self.label_telefono = Label(self.frame_nuevo_cliente, text="Telefono: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=2, sticky='s', padx=5, pady=5)
        self.telefono = Entry(self.frame_nuevo_cliente, width=25)
        self.telefono.grid(row=0, column=3, padx=10, pady=5)

        self.label_email = Label(self.frame_nuevo_cliente, text="E-mail: ", font=(
            "Arial", 10, "bold")).grid(row=1, column=2, sticky='s', padx=5, pady=5)
        self.email = Entry(self.frame_nuevo_cliente, width=25)
        self.email.grid(row=1, column=3, padx=10, pady=5)

        self.label_direccion = Label(self.frame_nuevo_cliente, text="Direccion: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=2, sticky='s', padx=5, pady=5)
        self.direccion = Entry(self.frame_nuevo_cliente, width=30)
        self.direccion.grid(row=2, column=3, padx=10, pady=5)

        "--------------- Frame botones --------------------"
        self.boton_registrar = Button(self.frame_nuevo_cliente, text="REGISTRAR", command=self.Agregar_cliente,
                                      height=1, width=15, bg="green", fg="white", font=("Arial", 10, "bold"))
        self.boton_registrar.grid(row=0, column=4, padx=10, pady=5)

        self.boton_editar = Button(self.frame_nuevo_cliente, text="EDITAR", command=self.Editar_cliente,
                                   height=1, width=15, bg="gray", fg="white", font=("Arial", 10, "bold"))
        self.boton_editar.grid(row=1, column=4, padx=10, pady=5)

        self.boton_eliminar = Button(self.frame_nuevo_cliente, text="ELIMINAR", command=self.Eliminar_cliente,
                                     height=1, width=15, bg="red", fg="white", font=("Arial", 10, "bold"))
        ...
        self.tree_cliente = ttk.Treeview(self.frame_tabla_clientes, height=11, columns=(
            "columna1", "columna2", "columna3", "columna4", "columna5"))
        ...
        self.tree_cliente.column(
            "columna1", width=150, minwidth=75, stretch=NO)

        self.tree_cliente.heading("columna2", text='Apellidos', anchor=CENTER)
        self.tree_cliente.column(
            "columna2", width=150, minwidth=75, stretch=NO)
        ...
        # "--------------- Buscar cliente --------------------"
        self.label_buscar_cliente = Label(self.frame_buscar_cliente, text="DNI: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=20, pady=10)
        self.buscar_dni = Entry(self.frame_buscar_cliente, width=30)
        self.buscar_dni.grid(row=0, column=1, padx=30, pady=5)
        self.boton_buscar = Button(self.frame_buscar_cliente, text="BUSCAR", command=self.Buscar_cliente,
                                   height=1, width=15, bg="black", fg="white", font=("Arial", 10, "bold"))
    ...
    def widgets_informacion(self):
        ...
        "--------------- Titulo --------------------"
        self.Label_titulo = Label(self.Label_informacion, text="APLICACION DE ESCRITORIO",
                                  fg="white", bg="black", font=("Arial", 23, "bold"), padx=150, pady=20)
        ...
        self.Label_titulo = Label(
            self.Label_informacion, text="> CONTROL DE PRODUCTOS ", fg="black", font=("Arial", 16, "bold"))
        self.Label_titulo.grid(row=2, column=0, sticky=W, padx=30, pady=10)

        self.Label_titulo = Label(
            self.Label_informacion, text="> BUSCADOR DE PRODUCTOS ", fg="black", font=("Arial", 16, "bold"))
        self.Label_titulo.grid(row=3, column=0, sticky=W, padx=30, pady=10)

        self.Label_titulo = Label(
            self.Label_informacion, text="> REGISTRO VENTAS ", fg="black", font=("Arial", 16, "bold"))
        self.Label_titulo.grid(row=4, column=0, sticky=W, padx=30, pady=10)

        self.Label_titulo = Label(
            self.Label_informacion, text="> GENERACION DE REPORTE ", fg="black", font=("Arial", 16, "bold"))
        self.Label_titulo.grid(row=5, column=0, sticky=W, padx=30, pady=10)

        self.Label_titulo = Label(
            self.Label_informacion, text="COATI CARE - 2023", fg="White", bg="black", font=("Arial", 22, "bold"))
    ...
    def widgets_nueva_venta(self):
        ...
        label_dni_venta = Label(self.frame_dni_venta, text="DNI: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=5)
        self.dni_venta = Entry(self.frame_dni_venta, width=25)
        self.dni_venta.focus()
        self.dni_venta.grid(row=0, column=1, padx=10, pady=5)

        label_medio_pago = Label(self.frame_dni_venta, text="Medio de pago: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=2, sticky='s', padx=5, pady=5)
        self.combo_medio_pago = ttk.Combobox(self.frame_dni_venta, values=[
                                             "Efectivo", "Tarjeta", "Transferencia"], width=22, state="readonly")
        ...
        label_codigo_producto_venta = Label(self.frame_nueva_venta, text="Codigo: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=5)
        ...
        label_cantidad_producto_venta = Label(self.frame_nueva_venta, text="Cantidad: ", font=(
            "Arial", 10, "bold")).grid(row=1, column=0, sticky='s', padx=5, pady=5)
        ...
        self.boton_agregar_producto_venta = Button(self.frame_nueva_venta, text="AGREGAR", command=self.Agregar_producto_venta,
                                                   height=1, width=15, bg="green", fg="white", font=("Arial", 10, "bold"))
        self.boton_agregar_producto_venta.grid(row=0, column=2, padx=5, pady=5)

        self.boton_eliminar_producto_venta = Button(self.frame_nueva_venta, text="ELIMINAR", command=self.Eliminar_producto_venta,
                                                    height=1, width=15, bg="red", fg="white", font=("Arial", 10, "bold"))
        self.boton_eliminar_producto_venta.grid(
        ...
        self.tree_nueva_venta = ttk.Treeview(self.frame_tabla_nueva_venta, height=11, columns=(
            "columna1", "columna2", "columna3", "columna4", "columna5"))
        self.tree_nueva_venta.heading("#0", text='Codigo', anchor=CENTER)
        self.tree_nueva_venta.column("#0", width=90, minwidth=75, stretch=NO)

        self.tree_nueva_venta.heading(
            "columna1", text='Producto', anchor=CENTER)
        self.tree_nueva_venta.column(
            "columna1", width=150, minwidth=75, stretch=NO)

        self.tree_nueva_venta.heading(
            "columna2", text='Descripcion', anchor=CENTER)
        self.tree_nueva_venta.column(
            "columna2", width=150, minwidth=75, stretch=NO)

        self.tree_nueva_venta.heading("columna3", text='Precio', anchor=CENTER)
        self.tree_nueva_venta.column(
            "columna3", width=70, minwidth=60, stretch=NO)

        self.tree_nueva_venta.heading(
            "columna4", text='Cantidad', anchor=CENTER)
        self.tree_nueva_venta.column(
            "columna4", width=70, minwidth=60, stretch=NO)

        self.tree_nueva_venta.heading(
            "columna5", text='Subtotal', anchor=CENTER)

        self.tree_nueva_venta.grid(row=0, column=0, sticky=E)

        "--------------- Finalizar venta --------------------"
        self.frame_finalizar_venta.config(bd=1)
        self.frame_finalizar_venta.grid(
            row=4, column=0, padx=5, pady=5, sticky=E)

        self.boton_finalizar_venta = Button(self.frame_finalizar_venta, text="Finalizar venta", command=self.Finalizar_venta,
                                            height=2, width=15, bg="black", fg="white", font=("Arial", 12, "bold"))
        self.boton_finalizar_venta.grid(row=0, column=0, padx=5, pady=5)

        label_venta_total = Label(self.frame_finalizar_venta, text="Venta total ($): ", font=(
            "Arial", 12, "bold")).grid(row=0, column=1, padx=5, pady=5)
        self.venta_total = Label(self.frame_finalizar_venta, text="0.00", font=(
            "Arial", 12, "bold"), height=1, width=10, bg="blue", fg="white")
    ...
    def widgets_ventas(self):
        ...
        self.label_buscar_venta = Label(self.frame_buscar_ventas, text="Buscar Por: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=5)
        self.combo_buscar_venta = ttk.Combobox(self.frame_buscar_ventas, values=[
                                               "DNI", "Fecha"], width=22, state="readonly")
        self.combo_buscar_venta.current(0)
        self.combo_buscar_venta.grid(row=0, column=1, padx=5, pady=5)

        label_dni_fecha = Label(self.frame_buscar_ventas, text="DNI / Fecha: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=2, sticky='s', padx=5, pady=5)
        ...
        "--------------- Boton --------------------"
        self.boton_buscar_ventas = Button(self.frame_boton_buscar_ventas, text="BUSCAR", command=self.Buscar_venta,
                                          height=2, width=20, bg="black", fg="white", font=("Arial", 10, "bold"))
        ...
        self.tree_buscar_ventas = ttk.Treeview(self.frame_tabla_ventas, height=11, columns=(
            "columna1", "columna2", "columna3", "columna4", "columna5"))
        self.tree_buscar_ventas.heading("#0", text='Fecha', anchor=CENTER)
        self.tree_buscar_ventas.column(
            "#0", width=150, minwidth=75, stretch=NO)

        self.tree_buscar_ventas.heading("columna1", text='DNI', anchor=CENTER)
        self.tree_buscar_ventas.column(
            "columna1", width=90, minwidth=60, stretch=NO)

        self.tree_buscar_ventas.heading(
            "columna2", text='Nombres', anchor=CENTER)
        self.tree_buscar_ventas.column(
            "columna2", width=90, minwidth=60, stretch=NO)

        self.tree_buscar_ventas.heading(
            "columna3", text='Apellidos', anchor=CENTER)
        self.tree_buscar_ventas.column(
            "columna3", width=90, minwidth=60, stretch=NO)

        self.tree_buscar_ventas.heading(
            "columna4", text='Medio de pago', anchor=CENTER)
        self.tree_buscar_ventas.column(
            "columna4", width=110, minwidth=75, stretch=NO)

        self.tree_buscar_ventas.heading(
            "columna5", text='Venta total', anchor=CENTER)
    ...
    def Agregar_producto(self):
        if self.Validar_formulario_completo() and self.Validar_registrar():
            ...
            parameters = (self.codigo.get(), self.nombre.get(), self.combo_categoria.get(
            ), self.cantidad.get(), self.precio.get(), self.descripcion.get())
            self.Ejecutar_consulta(query, parameters)
            messagebox.showinfo("REGISTRO EXITOSO", f'Producto registrado: {
                                self.nombre.get()}')
            print('REGISTRADO')
    ...
    def Eliminar_producto(self):
        ...
        respuesta = messagebox.askquestion(
            "ADVERTENCIA", f"¿Seguro que desea eliminar el producto: {nombre}?")
        if respuesta == 'yes':
            ...
            messagebox.showerror(
                'ERROR', f'Error al eliminar el producto: {nombre}')
    ...
    def Editar_producto(self):
        ...
        label_codigo = Label(self.Ventana_editar, text="Codigo del producto: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=8)
        nuevo_codigo = Entry(self.Ventana_editar, textvariable=StringVar(
            self.Ventana_editar, value=codigo), width=25)
        nuevo_codigo.grid(row=0, column=1, padx=5, pady=8)

        label_nombre = Label(self.Ventana_editar, text="Nombre del producto: ", font=(
            "Arial", 10, "bold")).grid(row=1, column=0, sticky='s', padx=5, pady=8)
        nuevo_nombre = Entry(self.Ventana_editar, textvariable=StringVar(
            self.Ventana_editar, value=nombre), width=25)
        nuevo_nombre.grid(row=1, column=1, padx=5, pady=8)

        label_categoria = Label(self.Ventana_editar, text="Categoria: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=0, sticky='s', padx=5, pady=9)
        nuevo_combo_categoria = ttk.Combobox(self.Ventana_editar, values=[
                                             "Componentes", "Perifericos"], width=22, state="readonly")
        nuevo_combo_categoria.set(categoria)
        nuevo_combo_categoria.grid(row=2, column=1, padx=5, pady=0)

        label_cantidad = Label(self.Ventana_editar, text="Cantidad: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=2, sticky='s', padx=5, pady=8)
        nueva_cantidad = Entry(self.Ventana_editar, textvariable=StringVar(
            self.Ventana_editar, value=cantidad), width=25)
        nueva_cantidad.grid(row=0, column=3, padx=5, pady=8)

        label_precio = Label(self.Ventana_editar, text="Precio ($): ", font=(
            "Arial", 10, "bold")).grid(row=1, column=2, sticky='s', padx=5, pady=8)
        nuevo_precio = Entry(self.Ventana_editar, textvariable=StringVar(
            self.Ventana_editar, value=precio), width=25)
        nuevo_precio.grid(row=1, column=3, padx=5, pady=8)

        label_descripcion = Label(self.Ventana_editar, text="Descripcion: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=2, sticky='s', padx=10, pady=8)
        nueva_descripcion = Entry(self.Ventana_editar, textvariable=StringVar(
            self.Ventana_editar, value=descripcion), width=25)
        nueva_descripcion.grid(row=2, column=3, padx=10, pady=8)

        boton_actualizar = Button(self.Ventana_editar, text="ACTUALIZAR", command=lambda: self.Actualizar(nuevo_codigo.get(), nuevo_nombre.get(), nuevo_combo_categoria.get(
        ), nueva_cantidad.get(), nuevo_precio.get(), nueva_descripcion.get(), codigo, nombre), height=2, width=20, bg="black", fg="white", font=("Arial", 10, "bold"))
    ...
    def Actualizar(self, nuevo_codigo, nuevo_nombre, nuevo_combo_categoria, nueva_cantidad, nuevo_precio, nueva_descripcion, codigo, nombre):
        ...
        parameters = (nuevo_codigo, nuevo_nombre, nuevo_combo_categoria,
                      nueva_cantidad, nuevo_precio, nueva_descripcion, codigo, nombre)
        self.Ejecutar_consulta(query, parameters)
    ...
    def Buscar_productos(self):
        if (self.Validar_busqueda()):
            ...
            if (self.combo_buscar.get() == 'Codigo'):
                ...
                for row in db_rows:
                    self.tree_buscar.insert("", 0, text=row[1], values=(
                        row[2], row[3], row[4], row[5], row[6]))
    ...
    def Validar_registrar(self):
        ...
        if (dato.fetchall() == []):
            ...
            messagebox.showerror("ERROR EN REGISTRO",
                                 "Codigo registrado anteriormente")
    ...
    def Obtener_clientes(self):
        ...
        db_rows = self.Ejecutar_consulta(query)
        for row in db_rows:
            self.tree_cliente.insert("", 0, text=row[0], values=(
                row[1], row[2], row[3], row[4], row[5]))

    def Agregar_cliente(self):
        if self.Validar_formulario_completo_cliente() and self.Validar_registrar_cliente():
            query = 'INSERT INTO Clientes VALUES(?, ?, ?, ?, ?, ?)'
            parameters = (self.dni.get(), self.nombres.get(), self.apellidos.get(
            ), self.telefono.get(), self.email.get(), self.direccion.get())
            self.Ejecutar_consulta(query, parameters)
            messagebox.showinfo("REGISTRO EXITOSO",
                                f'Cliente registrado: {self.dni.get()}')
            print('REGISTRADO')
    ...
    def Eliminar_cliente(self):
        ...
        nombre = self.tree_cliente.item(
            self.tree_cliente.selection())['values'][1]
        query = "DELETE FROM Clientes WHERE DNI = ?"
        respuesta = messagebox.askquestion(
            "ADVERTENCIA", f"¿Seguro que desea eliminar el cliente: {dato} - {nombre}?")
        if respuesta == 'yes':
            self.Ejecutar_consulta(query, (dato,))
            self.Obtener_clientes()
            messagebox.showinfo(
                'EXITO', f'Cliente eliminado: {dato} - {nombre}')
        else:
            messagebox.showerror(
                'ERROR', f'Error al eliminar el producto: {dato} - {nombre}')
    ...
    def Editar_cliente(self):
        ...
        nombres = self.tree_cliente.item(
            self.tree_cliente.selection())['values'][0]
        apellidos = self.tree_cliente.item(
            self.tree_cliente.selection())['values'][1]
        telefono = self.tree_cliente.item(
            self.tree_cliente.selection())['values'][2]
        email = self.tree_cliente.item(
            self.tree_cliente.selection())['values'][3]
        direccion = self.tree_cliente.item(
            self.tree_cliente.selection())['values'][4]
        ...
        # Valores ventana editar
        label_dni = Label(self.Ventana_editar_cliente, text="DNI: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=0, sticky='s', padx=5, pady=8)
        nuevo_dni = Entry(self.Ventana_editar_cliente, textvariable=StringVar(
            self.Ventana_editar_cliente, value=dni), width=25)
        nuevo_dni.grid(row=0, column=1, padx=5, pady=8)

        label_nombres = Label(self.Ventana_editar_cliente, text="Nombres: ", font=(
            "Arial", 10, "bold")).grid(row=1, column=0, sticky='s', padx=5, pady=8)
        nuevo_nombres = Entry(self.Ventana_editar_cliente, textvariable=StringVar(
            self.Ventana_editar_cliente, value=nombres), width=25)
        nuevo_nombres.grid(row=1, column=1, padx=5, pady=8)

        label_apellidos = Label(self.Ventana_editar_cliente, text="Apellidos: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=0, sticky='s', padx=5, pady=9)
        nuevo_apellidos = Entry(self.Ventana_editar_cliente, textvariable=StringVar(
            self.Ventana_editar_cliente, value=apellidos), width=25)
        nuevo_apellidos.grid(row=2, column=1, padx=5, pady=0)

        label_telefono = Label(self.Ventana_editar_cliente, text="Telefono: ", font=(
            "Arial", 10, "bold")).grid(row=0, column=2, sticky='s', padx=5, pady=8)
        nueva_telefono = Entry(self.Ventana_editar_cliente, textvariable=StringVar(
            self.Ventana_editar_cliente, value=telefono), width=25)
        nueva_telefono.grid(row=0, column=3, padx=5, pady=8)

        label_email = Label(self.Ventana_editar_cliente, text="E-mail: ", font=(
            "Arial", 10, "bold")).grid(row=1, column=2, sticky='s', padx=5, pady=8)
        nuevo_email = Entry(self.Ventana_editar_cliente, textvariable=StringVar(
            self.Ventana_editar_cliente, value=email), width=25)
        nuevo_email.grid(row=1, column=3, padx=5, pady=8)

        label_direccion = Label(self.Ventana_editar_cliente, text="Direccion: ", font=(
            "Arial", 10, "bold")).grid(row=2, column=2, sticky='s', padx=10, pady=8)
        nueva_direccion = Entry(self.Ventana_editar_cliente, textvariable=StringVar(
            self.Ventana_editar_cliente, value=direccion), width=25)
        nueva_direccion.grid(row=2, column=3, padx=10, pady=8)

        boton_actualizar_cliente = Button(self.Ventana_editar_cliente, text="ACTUALIZAR", command=lambda: self.Actualizar_cliente(nuevo_dni.get(), nuevo_nombres.get(), nuevo_apellidos.get(
        ), nueva_telefono.get(), nuevo_email.get(), nueva_direccion.get(), dni, nombres), height=2, width=20, bg="black", fg="white", font=("Arial", 10, "bold"))
        boton_actualizar_cliente.grid(
    ...
    def Actualizar_cliente(self, nuevo_dni, nuevo_nombres, nuevo_apelllidos, nueva_telefono, nuevo_email, nueva_direccion, dni, nombres):
        ...
        parameters = (nuevo_dni, nuevo_nombres, nuevo_apelllidos,
                      nueva_telefono, nuevo_email, nueva_direccion, dni, nombres)
        self.Ejecutar_consulta(query, parameters)
    ...
    def Agregar_producto_venta(self):
        ...
        if (self.Validar_busqueda_producto_venta()):
            query = (
                "SELECT Codigo,Nombre,Descripcion,Precio FROM Productos WHERE Codigo LIKE ?")
            ...
            print(rows[2])
            self.tree_nueva_venta.insert("", 0, text=rows[0][0], values=(
                rows[0][1], rows[0][2], rows[0][3], rows[1], rows[2]))
    ...
    def Finalizar_venta(self):
        ...
        parameters = (self.dni_venta.get(), formato_fecha,
                      self.combo_medio_pago.get(), self.monto_total)
        print('REGISTRADO')
        respuesta = messagebox.askquestion(
            "ADVERTENCIA", f"¿Seguro que desea terminar la venta?")
        if respuesta == 'yes':
            self.Ejecutar_consulta(query, parameters)
            messagebox.showinfo("VENTA FINALIZADA",
                                f'Monto total de ven: {self.monto_total}')
    ...
    def Buscar_venta(self):
        if (self.Validar_busqueda_ventas()):
            ...
            if (self.combo_buscar_venta.get() == 'DNI'):
                ...
                db_rows = self.Ejecutar_consulta(query, (parameters,))
                for row in db_rows:
                    self.tree_buscar_ventas.insert("", 0, text=row[0], values=(
                        row[1], row[2], row[3], row[4], row[5]))
    ...
    def Eliminar_producto_venta(self):
        try:
            ...
            return
        respuesta = messagebox.askquestion(
            "ADVERTENCIA", f"¿Seguro que desea eliminar el producto?")
        if respuesta == 'yes':
    ...
    def Validar_busqueda_ventas(self):
        if len(self.dni_fecha.get()) != 0:
            ...
            self.tree_buscar_ventas.delete(
                *self.tree_buscar_ventas.get_children())
            messagebox.showerror(
                "ERROR", "Complete todos los campos para la busqueda")
